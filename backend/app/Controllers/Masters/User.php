...
class User extends Home
{
     ...
     */
    public function user_master_form(): Response
    {
        try {
    ...
    public function add_user(): Response
    {
        try {
            ...
            $db = db_connect();
            $db->transBegin();
            // Get user data with UUID directly
            ...
            // Prepare data excluding role_id
            $data = [];
            foreach ($user_data as $key => $value) {
                $data[$key] = trim($value);
            }
            // Check if data is empty after excluding role_id
            if (empty($data)) {
                return $this->response->setJSON($this->makeOutput(array(), FAIL_ERR_CODE, DATA_REQUIRED));
            }
            ...
            // Set 'added_by' field to the ID of the user performing the addition
            $form_rules['added_by'] = [
                'label' => 'Added By',
                "rules" => ['required'],
            ];
            // Generate a random password and hash it using md5
            $form_rules['password'] = [
                'label' => 'Password',
                "rules" => ['required', 'max_length[50]'],
            ];
            // Generating random password the user will change the password on first login
